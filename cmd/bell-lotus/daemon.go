...
var DaemonCmd = &cli.Command{
	...
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:     "mgo-bstore",
			Usage:    "mongo dsn for chain blockstore",
			Required: true,
		},
		&cli.BoolFlag{
			Name:  "mgo-bstore-sync",
			Usage: "mongo inserting mode for chain blockstore",
		},
		&cli.BoolFlag{
			Name:  "mgo-bstore-readonly",
			Usage: "mongo r/w mode for chain blockstore",
		},
		&cli.StringFlag{
			Name:     "mgo-metads",
			Usage:    "mongo dsn for metadata ds",
			Required: true,
		},
		&cli.BoolFlag{
			Name:  "mgo-metads-readonly",
			Usage: "mongo r/w mode for metadata ds",
		},
		&cli.StringFlag{
			Name:  "api",
			Value: "1234",
	...
	Action: func(cctx *cli.Context) error {
		...
		if chainfile != "" || snapshot != "" {
			...
			mbs, err := mbstore.NewMgoBstore(ctx, cctx.String("mgo-bstore"), mbstore.MgoPutSync(true))
			if err != nil {
				return err
			}

			mdscli, err := mgoutil.Connect(cctx.Context, cctx.String("mgo-metads"))
			if err != nil {
				return err
			}

			mds, err := mmetads.NewMgoDS(cctx.Context, mdscli, false)
			if err != nil {
				return err
			}
		...
		stop, err := node.New(ctx,
			...
			// defined & used only in bell-lotus
			node.Override(new(dep.MgoMetaDSDSN), dep.MgoMetaDSDSN(cctx.String("mgo-metads"))),
			node.Override(new(dep.MgoMetaDSReadOnly), dep.MgoMetaDSReadOnly(cctx.IsSet("mgo-metads-readonly"))),
			node.Override(new(dtypes.MetadataDS), dep.MgoMetaDS),
