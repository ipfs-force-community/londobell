...
func GetAggPreNetFee(c *gin.Context) {
	...
	// multi dbs query
	{
		multiResult, err := multiquery.MultiRangeQuery(ctx, req.StartEpoch, req.EndEpoch, countUtils, common2.AggPreNetfeeAggregator, req, "ExecTrace")
		if err != nil {
			alog.Error(err)
			util.ReturnOnErr(c, err)
			return
		...
		err = json.Unmarshal(rawByte, &aggPreNetFeeRes)
		if err != nil {
			alog.Error(err)
			util.ReturnOnErr(c, err)
			return
