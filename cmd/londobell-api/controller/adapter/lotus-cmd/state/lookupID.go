...
func GetStateLookupID(c *gin.Context) {
	alog := adapter.Log.With("method", "GetStateLookupID")
	req := lotusCmdModel.StateLookupIDReq{}
	...
	var lookupAddr address.Address
	if !req.Reverse {
		lookupAddr, err = api.StateLookupID(ctx, addr, ts.Key())
	} else {
		lookupAddr, err = api.StateAccountKey(ctx, addr, ts.Key())
	}
	...
	res.Data = lotusCmdModel.StateLookupIDRes{
		Epoch:      ts.Height(),
		Addr:       addr,
