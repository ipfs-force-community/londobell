...
func GetStateReadState(c *gin.Context) {
	alog := adapter.Log.With("method", "GetStateReadState")
	req := lotusCmdModel.StateReadStateReq{}
	...
	actor, err := address.NewFromString(req.Actor)
	if err != nil {
		util.ReturnOnErr(c, alog, err)
		return
	}
	...
	as, err := api.StateReadState(ctx, actor, ts.Key())
	if err != nil {
		util.ReturnOnErr(c, alog, err)
		return
	}

	res.Data = lotusCmdModel.StateReadStateRes{
		Epoch:      ts.Height(),
		Actor:      actor,
		ActorState: as,
	}
