...
import (
	...
	util16 "github.com/filecoin-project/go-state-types/builtin/v16/util"
	...
	util17 "github.com/filecoin-project/go-state-types/builtin/v17/util"
	...
	util18 "github.com/filecoin-project/go-state-types/builtin/v18/util"
...
func MakeSystemState(store adt.Store, c cid.Cid) (system.State, error) {
	if name, av, ok := actors.GetActorMetaByCode(c); ok {
		...
		case actorstypes.Version16:
			return system.MakeState(store, actorstypes.Version16, cid.Undef)
		case actorstypes.Version17:
			return system.MakeState(store, actorstypes.Version17, cid.Undef)
		case actorstypes.Version18:
			return system.MakeState(store, actorstypes.Version18, cid.Undef)
		}
	}
...
func getMinerResByCode(ctx context.Context, mact *types.Actor, stor adt.Store, resData *model.MinerRes) (err error) {
	...
	if name, av, ok := actors.GetActorMetaByCode(mact.Code); ok {
		...
		case actorstypes.Version16:
			...
			err = dls.ForEach(stor, func(dlIdx uint64, dl *miner16.Deadline) error {
				partitions, err := dl.PartitionsArray(stor)
				if err != nil {
					return err
				}
			...
			precommitted, err := adt16.AsMap(stor, state.PreCommittedSectors, sbuiltin.DefaultHamtBitwidth)
			if err != nil {
				return err
			}
		...
		case actorstypes.Version17:
			...
			err = dls.ForEach(stor, func(dlIdx uint64, dl *miner17.Deadline) error {
				partitions, err := dl.PartitionsArray(stor)
				if err != nil {
					return err
				}
			...
			precommitted, err := adt17.AsMap(stor, state.PreCommittedSectors, sbuiltin.DefaultHamtBitwidth)
			if err != nil {
				return err
			}
		...
		case actorstypes.Version18:
			...
			err = dls.ForEach(stor, func(dlIdx uint64, dl *miner18.Deadline) error {
				partitions, err := dl.PartitionsArray(stor)
				if err != nil {
					return err
				}
			...
			precommitted, err := adt18.AsMap(stor, state.PreCommittedSectors, sbuiltin.DefaultHamtBitwidth)
			if err != nil {
				return err
			}
...
func CleanUpExpiredPreCommits(state interface{}, store adt.Store, currEpoch abi.ChainEpoch) (depositToBurn abi.TokenAmount, err error) {
	...
	case *miner16.State:
		st := state.(*miner16.State)
		depositToBurn = abi.NewTokenAmount(0)

		// cleanup expired pre-committed sectors
		cleanUpQ, err := util16.LoadBitfieldQueue(store, st.PreCommittedSectorsCleanUp, st.QuantSpecEveryDeadline(), miner16.PrecommitCleanUpAmtBitwidth)
		if err != nil {
			return depositToBurn, xerrors.Errorf("failed to load sector expiry queue: %w", err)
		}
	...
	case *miner17.State:
		st := state.(*miner17.State)
		depositToBurn = abi.NewTokenAmount(0)

		// cleanup expired pre-committed sectors
		cleanUpQ, err := util17.LoadBitfieldQueue(store, st.PreCommittedSectorsCleanUp, st.QuantSpecEveryDeadline(), miner17.PrecommitCleanUpAmtBitwidth)
		if err != nil {
			return depositToBurn, xerrors.Errorf("failed to load sector expiry queue: %w", err)
		}
	...
	case *miner18.State:
		st := state.(*miner18.State)
		depositToBurn = abi.NewTokenAmount(0)

		// cleanup expired pre-committed sectors
		cleanUpQ, err := util18.LoadBitfieldQueue(store, st.PreCommittedSectorsCleanUp, st.QuantSpecEveryDeadline(), miner18.PrecommitCleanUpAmtBitwidth)
		if err != nil {
			return depositToBurn, xerrors.Errorf("failed to load sector expiry queue: %w", err)
		}
...
func PopUntil(queue interface{}, until abi.ChainEpoch) (values bitfield.BitField, modified bool, err error) {
	...
	case util15.BitfieldQueue:
		...
		return merged, true, nil
	case util16.BitfieldQueue:
		q := queue.(util16.BitfieldQueue)
		if err = q.ForEach(func(epoch abi.ChainEpoch, bf bitfield.BitField) error {
			if epoch > until {
				return stopErr
			}
		...
		return merged, true, nil
	case util17.BitfieldQueue:
		q := queue.(util17.BitfieldQueue)
		if err = q.ForEach(func(epoch abi.ChainEpoch, bf bitfield.BitField) error {
			if epoch > until {
				return stopErr
			}
		...
		return merged, true, nil
	case util18.BitfieldQueue:
		q := queue.(util18.BitfieldQueue)
		if err = q.ForEach(func(epoch abi.ChainEpoch, bf bitfield.BitField) error {
			if epoch > until {
				return stopErr
			}
...
func DeletePrecommittedSectors(state interface{}, store adt.Store, sectorNos ...abi.SectorNumber) error {
	...
	case *miner16.State:
		st := state.(*miner16.State)
		precommitted, err := adt16.AsMap(store, st.PreCommittedSectors, sbuiltin.DefaultHamtBitwidth)
		if err != nil {
			return err
		}

		for _, sectorNo := range sectorNos {
			err = precommitted.Delete(miner16.SectorKey(sectorNo))
			if err != nil {
				return xerrors.Errorf("failed to delete precommitment for %v: %w", sectorNo, err)
			}
		}
	...
	case *miner17.State:
		st := state.(*miner17.State)
		precommitted, err := adt17.AsMap(store, st.PreCommittedSectors, sbuiltin.DefaultHamtBitwidth)
		if err != nil {
			return err
		}

		for _, sectorNo := range sectorNos {
			err = precommitted.Delete(miner17.SectorKey(sectorNo))
			if err != nil {
				return xerrors.Errorf("failed to delete precommitment for %v: %w", sectorNo, err)
			}
		}
	...
	case *miner18.State:
		st := state.(*miner18.State)
		precommitted, err := adt18.AsMap(store, st.PreCommittedSectors, sbuiltin.DefaultHamtBitwidth)
		if err != nil {
			return err
		}

		for _, sectorNo := range sectorNos {
			err = precommitted.Delete(miner18.SectorKey(sectorNo))
			if err != nil {
				return xerrors.Errorf("failed to delete precommitment for %v: %w", sectorNo, err)
			}
		}
