...
func init() {
	summaryDaysV2 = []abi.ChainEpoch{1, 7, 14, 30}
	for d := (miner2.MinSectorExpiration / builtin2.EpochsInDay); d <= (miner2.MaxSectorExpirationExtension / builtin2.EpochsInDay); d += 180 {
		summaryDaysV2 = append(summaryDaysV2, abi.ChainEpoch(d))
	}

	mustRegisterRegularExtractor("MinerSectorSummaryV2", extractMinerSectorSummaryV2)
}

var summaryDaysV2 []abi.ChainEpoch

func extractMinerSectorSummaryV2(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *miner2.State) error {
	...
	if st.Sectors.Equals(emptyMinerStateV2.Sectors) {
		return nil
	}

	daysMax := summaryDaysV2[len(summaryDaysV2)-1]
	summaries := make([]*model.MinerSectorSummaryRange, 0, len(summaryDaysV2)+1)
	summariesInDays := make([]*model.MinerSectorSummaryRange, 0, int(daysMax+1))

	var prevDays abi.ChainEpoch
	for _, days := range summaryDaysV2 {
		current := &model.MinerSectorSummaryRange{
			LowerBound:              prevDays * builtin2.EpochsInDay,
			UpperBound:              days * builtin2.EpochsInDay,
			SectorCount:             0,
			DealCount:               0,
			V1SectorCount:           0,
	...
	sectors, err := miner2.LoadSectors(adt2.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Sectors)
	if err != nil {
	...
	err = sectors.ForEach(&out, func(n int64) error {
		...
		remainDays := remainDuration / builtin2.EpochsInDay
		...
		if len(out.DealIDs) == 0 {
			...
			mds = append(mds, model.MinerDealSector{
				...
				QAPower:            miner2.QAPowerForSector(sectorSize, &out),
				Miner:              head.Addr,
			})
		}
		return nil
