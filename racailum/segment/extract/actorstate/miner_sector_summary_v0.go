...
func init() {
	summaryDaysV0 = []abi.ChainEpoch{1, 7, 14, 30}
...
func extractMinerSectorSummaryV0(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *miner0.State) error {
	...
	daysMax := summaryDaysV0[len(summaryDaysV0)-1]
	summaries := make([]*model.MinerSectorSummaryRange, 0, len(summaryDaysV0)+1)
	summariesInDays := make([]*model.MinerSectorSummaryRange, 0, int(daysMax+1))

	var prevDays abi.ChainEpoch
	for _, days := range summaryDaysV0 {
		current := &model.MinerSectorSummaryRange{
			LowerBound:              prevDays * builtin0.EpochsInDay,
			UpperBound:              days * builtin0.EpochsInDay,
			SectorCount:             0,
			DealCount:               0,
			V1SectorCount:           0,
	...
	sectors, err := miner0.LoadSectors(adt0.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Sectors)
	if err != nil {
	...
	err = sectors.ForEach(&out, func(n int64) error {
		...
		remainDays := remainDuration / builtin0.EpochsInDay
		...
		if len(out.DealIDs) == 0 {
			...
			mds = append(mds, model.MinerDealSector{
				...
				QAPower:            miner0.QAPowerForSector(sectorSize, &out),
				Miner:              head.Addr,
			})
		}
		return nil
