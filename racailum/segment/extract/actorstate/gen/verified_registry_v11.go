...
func init() {
	reg.MustRegisterPreCheck("VerifRegV11", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("VerifRegV11", extractVerifRegV11)

}

func extractVerifRegV11(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *verifreg11.State) error {
	mapRoots := []struct {
		name string
		root cid.Cid
	}{
	...
	for mi := range mapRoots {
		...
		m, err := adt11.AsMap(ctx.D.ActorStore(ctx.C), root, builtin11.DefaultHamtBitwidth)
		if err != nil {
