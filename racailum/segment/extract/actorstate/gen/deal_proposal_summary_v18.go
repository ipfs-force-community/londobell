...
func init() {
	reg.MustRegisterPreCheck("DealProposalSummaryV18", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalSummaryV18", extractDealProposalSummaryV18)

}

func extractDealProposalSummaryV18(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market18.State) error {
	deals, err := market18.AsDealProposalArray(adt18.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Proposals)
	if err != nil {
	...
	addProposal := func(p market18.DealProposal) {
		target := 0
		if p.VerifiedDeal {
			target = 1
		}
