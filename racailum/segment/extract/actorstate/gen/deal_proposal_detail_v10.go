...
func init() {
	reg.MustRegisterPreCheck("DealProposalDetailedV10", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalDetailedV10", extractDealProposalDetailedV10)

}

func extractDealProposalDetailedV10(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market10.State) error {
	...
	state, err := lmarket.Load(adt10.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), head.Actor)
	if err != nil {
