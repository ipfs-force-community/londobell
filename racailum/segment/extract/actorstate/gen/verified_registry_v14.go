...
func init() {
	reg.MustRegisterPreCheck("VerifRegV14", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("VerifRegV14", extractVerifRegV14)

}

func extractVerifRegV14(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *verifreg14.State) error {
	mapRoots := []struct {
		name string
		root cid.Cid
	}{
	...
	for mi := range mapRoots {
		...
		m, err := adt14.AsMap(ctx.D.ActorStore(ctx.C), root, builtin14.DefaultHamtBitwidth)
		if err != nil {
