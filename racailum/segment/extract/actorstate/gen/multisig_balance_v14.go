...
func init() {
	reg.MustRegisterPreCheck("MultisigBalanceV14", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MultisigBalance
	}, nil)
	reg.MustRegisterRegularExtractor("MultisigBalanceV14", extractMultisigBalanceV14)

}

func extractMultisigBalanceV14(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *multisig14.State) error {
	epoch := head.Epoch
