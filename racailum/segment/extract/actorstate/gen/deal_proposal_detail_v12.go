...
func init() {
	reg.MustRegisterPreCheck("DealProposalDetailedV12", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalDetailedV12", extractDealProposalDetailedV12)

}

func extractDealProposalDetailedV12(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market12.State) error {
	...
	state, err := lmarket.Load(adt12.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), head.Actor)
	if err != nil {
