...
func init() {
	reg.MustRegisterPreCheck("DealProposalDetailedV18", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalDetailedV18", extractDealProposalDetailedV18)

}

func extractDealProposalDetailedV18(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market18.State) error {
	...
	state, err := lmarket.Load(adt18.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), head.Actor)
	if err != nil {
