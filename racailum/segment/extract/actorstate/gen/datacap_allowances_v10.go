...
func init() {
	reg.MustRegisterPreCheck("DatacapAllowancesV10", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DatacapAllowancesV10", extractDatacapAllowancesV10)

	schema.Register(
		schema.Model{
			Name: "datacap-allowances-v10",
			D:    &model.DatacapAllowances{},
		},
	)
}

func extractDatacapAllowancesV10(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *datacap10.State) error {
	actorToHamtMap, err := adt10.AsMap(ctx.D.ActorStore(ctx.C), st.Token.Allowances, int(st.Token.HamtBitWidth))
	if err != nil {
	...
	err = actorToHamtMap.ForEach(&innerHamtCid, func(key string) error {
		innerMap, err := adt10.AsMap(ctx.D.ActorStore(ctx.C), cid.Cid(innerHamtCid), int(st.Token.HamtBitWidth))
		if err != nil {
