...
func init() {
	summaryDaysV15 = []abi.ChainEpoch{1, 7, 14, 30}
	for d := (miner15.MinSectorExpiration / builtin15.EpochsInDay); d <= (miner15.MaxSectorExpirationExtension / builtin15.EpochsInDay); d += 180 {
		summaryDaysV15 = append(summaryDaysV15, abi.ChainEpoch(d))
	}
	if (miner15.MaxSectorExpirationExtension / builtin15.EpochsInDay) > summaryDaysV15[len(summaryDaysV15)-1] {
		summaryDaysV15 = append(summaryDaysV15, abi.ChainEpoch(miner15.MaxSectorExpirationExtension/builtin15.EpochsInDay))
	}

	reg.MustRegisterPreCheck("MinerSectorSummaryV15", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("MinerSectorSummaryV15", extractMinerSectorSummaryV15)
}

var summaryDaysV15 []abi.ChainEpoch

func extractMinerSectorSummaryV15(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *miner15.State) error {
	if st.Sectors.Equals(emptyMinerStateV15.Sectors) {
		return nil
	}

	daysMax := summaryDaysV15[len(summaryDaysV15)-1]
	summaries := make([]*model.MinerSectorSummaryRange, 0, len(summaryDaysV15)+1)
	summariesInDays := make([]*model.MinerSectorSummaryRange, 0, int(daysMax+1))

	var prevDays abi.ChainEpoch
	for _, days := range summaryDaysV15 {
		current := &model.MinerSectorSummaryRange{
			LowerBound:              prevDays * builtin15.EpochsInDay,
			UpperBound:              days * builtin15.EpochsInDay,
			SectorCount:             0,
			DealCount:               0,
			V1SectorCount:           0,
	...
	sectors, err := miner15.LoadSectors(adt15.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Sectors)
	if err != nil {
		return fmt.Errorf("load sectors from adt store: %w", err)
	}

	var out miner15.SectorOnChainInfo
	...
	err = sectors.ForEach(&out, func(n int64) error {
		...
		remainDays := remainDuration / builtin15.EpochsInDay
		...
		if len(out.DealIDs) == 0 {
			...
			mds = append(mds, model.MinerDealSector{
				...
				QAPower:            miner15.QAPowerForWeight(sectorSize, out.Expiration-out.Activation, out.VerifiedDealWeight),
				Miner:              head.Addr,
			})
		}
		return nil
