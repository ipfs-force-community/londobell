...
func init() {
	summaryDaysV8 = []abi.ChainEpoch{1, 7, 14, 30}
	for d := (miner8.MinSectorExpiration / builtin8.EpochsInDay); d <= (miner8.MaxSectorExpirationExtension / builtin8.EpochsInDay); d += 180 {
		summaryDaysV8 = append(summaryDaysV8, abi.ChainEpoch(d))
	}
	if (miner8.MaxSectorExpirationExtension / builtin8.EpochsInDay) > summaryDaysV8[len(summaryDaysV8)-1] {
		summaryDaysV8 = append(summaryDaysV8, abi.ChainEpoch(miner8.MaxSectorExpirationExtension/builtin8.EpochsInDay))
	}

	reg.MustRegisterPreCheck("MinerSectorSummaryV8", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("MinerSectorSummaryV8", extractMinerSectorSummaryV8)
}

var summaryDaysV8 []abi.ChainEpoch

func extractMinerSectorSummaryV8(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *miner8.State) error {
	if st.Sectors.Equals(emptyMinerStateV8.Sectors) {
		return nil
	}

	daysMax := summaryDaysV8[len(summaryDaysV8)-1]
	summaries := make([]*model.MinerSectorSummaryRange, 0, len(summaryDaysV8)+1)
	summariesInDays := make([]*model.MinerSectorSummaryRange, 0, int(daysMax+1))

	var prevDays abi.ChainEpoch
	for _, days := range summaryDaysV8 {
		current := &model.MinerSectorSummaryRange{
			LowerBound:              prevDays * builtin8.EpochsInDay,
			UpperBound:              days * builtin8.EpochsInDay,
			SectorCount:             0,
			DealCount:               0,
			V1SectorCount:           0,
	...
	err = sectors.ForEach(&out, func(n int64) error {
		...
		remainDays := remainDuration / builtin8.EpochsInDay
