...
func init() {
	reg.MustRegisterPreCheck("MarketFundsV14", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MarketFunds
	}, func(ctx *extract.Ctx) int {
		return ctx.Opts.StateRegular.MarketFundsTicks
	})
	reg.MustRegisterRegularExtractor("MarketFundsV14", extractMarketFundsV14)

}

func extractMarketFundsV14(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market14.State) error {
	...
	proposals, err := market14.AsDealProposalArray(actStore, st.Proposals)
	if err != nil {
		return fmt.Errorf("constrcut DealProposalArray: %w", err)
	}

	deal := &market14.DealProposal{}
