...
func init() {
	reg.MustRegisterPreCheck("MultisigBalanceV16", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MultisigBalance
	}, nil)
	reg.MustRegisterRegularExtractor("MultisigBalanceV16", extractMultisigBalanceV16)

}

func extractMultisigBalanceV16(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *multisig16.State) error {
	epoch := head.Epoch
