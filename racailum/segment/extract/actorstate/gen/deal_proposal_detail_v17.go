...
func init() {
	reg.MustRegisterPreCheck("DealProposalDetailedV17", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalDetailedV17", extractDealProposalDetailedV17)

}

func extractDealProposalDetailedV17(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market17.State) error {
	...
	state, err := lmarket.Load(adt17.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), head.Actor)
	if err != nil {
