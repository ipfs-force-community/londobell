...
func init() {
	reg.MustRegisterPreCheck("VerifRegV15", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("VerifRegV15", extractVerifRegV15)

}

func extractVerifRegV15(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *verifreg15.State) error {
	mapRoots := []struct {
		name string
		root cid.Cid
	}{
	...
	for mi := range mapRoots {
		...
		m, err := adt15.AsMap(ctx.D.ActorStore(ctx.C), root, builtin15.DefaultHamtBitwidth)
		if err != nil {
