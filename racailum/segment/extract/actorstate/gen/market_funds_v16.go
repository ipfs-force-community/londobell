...
func init() {
	reg.MustRegisterPreCheck("MarketFundsV16", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MarketFunds
	}, func(ctx *extract.Ctx) int {
		return ctx.Opts.StateRegular.MarketFundsTicks
	})
	reg.MustRegisterRegularExtractor("MarketFundsV16", extractMarketFundsV16)

}

func extractMarketFundsV16(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market16.State) error {
	...
	proposals, err := market16.AsDealProposalArray(actStore, st.Proposals)
	if err != nil {
		return fmt.Errorf("constrcut DealProposalArray: %w", err)
	}

	deal := &market16.DealProposal{}
