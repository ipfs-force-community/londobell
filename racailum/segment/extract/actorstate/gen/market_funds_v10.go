...
func init() {
	reg.MustRegisterPreCheck("MarketFundsV10", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MarketFunds
	}, func(ctx *extract.Ctx) int {
		return ctx.Opts.StateRegular.MarketFundsTicks
	})
	reg.MustRegisterRegularExtractor("MarketFundsV10", extractMarketFundsV10)

}

func extractMarketFundsV10(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market10.State) error {
	...
	proposals, err := market10.AsDealProposalArray(actStore, st.Proposals)
	if err != nil {
		return fmt.Errorf("constrcut DealProposalArray: %w", err)
	}

	deal := &market10.DealProposal{}
