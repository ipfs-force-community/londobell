...
func init() {
	summaryDaysV16 = []abi.ChainEpoch{1, 7, 14, 30}
	for d := (miner16.MinSectorExpiration / builtin16.EpochsInDay); d <= (miner16.MaxSectorExpirationExtension / builtin16.EpochsInDay); d += 180 {
		summaryDaysV16 = append(summaryDaysV16, abi.ChainEpoch(d))
	}
	if (miner16.MaxSectorExpirationExtension / builtin16.EpochsInDay) > summaryDaysV16[len(summaryDaysV16)-1] {
		summaryDaysV16 = append(summaryDaysV16, abi.ChainEpoch(miner16.MaxSectorExpirationExtension/builtin16.EpochsInDay))
	}

	reg.MustRegisterPreCheck("MinerSectorSummaryV16", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("MinerSectorSummaryV16", extractMinerSectorSummaryV16)
}

var summaryDaysV16 []abi.ChainEpoch

func extractMinerSectorSummaryV16(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *miner16.State) error {
	if st.Sectors.Equals(emptyMinerStateV16.Sectors) {
		return nil
	}

	daysMax := summaryDaysV16[len(summaryDaysV16)-1]
	summaries := make([]*model.MinerSectorSummaryRange, 0, len(summaryDaysV16)+1)
	summariesInDays := make([]*model.MinerSectorSummaryRange, 0, int(daysMax+1))

	var prevDays abi.ChainEpoch
	for _, days := range summaryDaysV16 {
		current := &model.MinerSectorSummaryRange{
			LowerBound:              prevDays * builtin16.EpochsInDay,
			UpperBound:              days * builtin16.EpochsInDay,
			SectorCount:             0,
			DealCount:               0,
			V1SectorCount:           0,
	...
	sectors, err := miner16.LoadSectors(adt16.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Sectors)
	if err != nil {
		return fmt.Errorf("load sectors from adt store: %w", err)
	}

	var out miner16.SectorOnChainInfo
	...
	err = sectors.ForEach(&out, func(n int64) error {
		...
		remainDays := remainDuration / builtin16.EpochsInDay
		...
		if len(out.DeprecatedDealIDs) == 0 {
			...
			mds = append(mds, model.MinerDealSector{
				...
				QAPower:            miner16.QAPowerForWeight(sectorSize, out.Expiration-out.Activation, out.VerifiedDealWeight),
				Miner:              head.Addr,
			})
		}
		return nil
