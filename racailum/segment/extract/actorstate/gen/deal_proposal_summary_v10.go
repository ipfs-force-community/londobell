...
func init() {
	reg.MustRegisterPreCheck("DealProposalSummaryV10", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalSummaryV10", extractDealProposalSummaryV10)

}

func extractDealProposalSummaryV10(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market10.State) error {
	deals, err := market10.AsDealProposalArray(adt10.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Proposals)
	if err != nil {
	...
	addProposal := func(p market10.DealProposal) {
		target := 0
		if p.VerifiedDeal {
			target = 1
		}
