...
func init() {
	reg.MustRegisterPreCheck("MarketFundsV18", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MarketFunds
	}, func(ctx *extract.Ctx) int {
		return ctx.Opts.StateRegular.MarketFundsTicks
	})
	reg.MustRegisterRegularExtractor("MarketFundsV18", extractMarketFundsV18)

}

func extractMarketFundsV18(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market18.State) error {
	...
	proposals, err := market18.AsDealProposalArray(actStore, st.Proposals)
	if err != nil {
		return fmt.Errorf("constrcut DealProposalArray: %w", err)
	}

	deal := &market18.DealProposal{}
