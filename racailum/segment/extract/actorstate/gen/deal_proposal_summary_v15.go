...
func init() {
	reg.MustRegisterPreCheck("DealProposalSummaryV15", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalSummaryV15", extractDealProposalSummaryV15)

}

func extractDealProposalSummaryV15(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market15.State) error {
	deals, err := market15.AsDealProposalArray(adt15.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Proposals)
	if err != nil {
	...
	addProposal := func(p market15.DealProposal) {
		target := 0
		if p.VerifiedDeal {
			target = 1
		}
