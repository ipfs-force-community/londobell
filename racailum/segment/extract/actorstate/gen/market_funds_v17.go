...
func init() {
	reg.MustRegisterPreCheck("MarketFundsV17", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MarketFunds
	}, func(ctx *extract.Ctx) int {
		return ctx.Opts.StateRegular.MarketFundsTicks
	})
	reg.MustRegisterRegularExtractor("MarketFundsV17", extractMarketFundsV17)

}

func extractMarketFundsV17(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market17.State) error {
	...
	proposals, err := market17.AsDealProposalArray(actStore, st.Proposals)
	if err != nil {
		return fmt.Errorf("constrcut DealProposalArray: %w", err)
	}

	deal := &market17.DealProposal{}
