...
func init() {
	reg.MustRegisterPreCheck("DealProposalSummaryV17", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalSummaryV17", extractDealProposalSummaryV17)

}

func extractDealProposalSummaryV17(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market17.State) error {
	deals, err := market17.AsDealProposalArray(adt17.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Proposals)
	if err != nil {
	...
	addProposal := func(p market17.DealProposal) {
		target := 0
		if p.VerifiedDeal {
			target = 1
		}
