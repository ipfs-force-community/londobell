...
func init() {
	reg.MustRegisterPreCheck("DealProposalSummaryV7", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalSummaryV7", extractDealProposalSummaryV7)
...
func extractDealProposalSummaryV7(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market7.State) error {
	...
	addProposal := func(p market7.DealProposal) {
		target := 0
		if p.VerifiedDeal {
			target = 1
		}
