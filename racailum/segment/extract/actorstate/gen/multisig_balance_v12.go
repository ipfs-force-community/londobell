...
func init() {
	reg.MustRegisterPreCheck("MultisigBalanceV12", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MultisigBalance
	}, nil)
	reg.MustRegisterRegularExtractor("MultisigBalanceV12", extractMultisigBalanceV12)

}

func extractMultisigBalanceV12(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *multisig12.State) error {
	epoch := head.Epoch
