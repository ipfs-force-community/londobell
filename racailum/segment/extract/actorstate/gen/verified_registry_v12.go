...
func init() {
	reg.MustRegisterPreCheck("VerifRegV12", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("VerifRegV12", extractVerifRegV12)

}

func extractVerifRegV12(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *verifreg12.State) error {
	mapRoots := []struct {
		name string
		root cid.Cid
	}{
	...
	for mi := range mapRoots {
		...
		m, err := adt12.AsMap(ctx.D.ActorStore(ctx.C), root, builtin12.DefaultHamtBitwidth)
		if err != nil {
