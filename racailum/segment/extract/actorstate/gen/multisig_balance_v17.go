...
func init() {
	reg.MustRegisterPreCheck("MultisigBalanceV17", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MultisigBalance
	}, nil)
	reg.MustRegisterRegularExtractor("MultisigBalanceV17", extractMultisigBalanceV17)

}

func extractMultisigBalanceV17(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *multisig17.State) error {
	epoch := head.Epoch
