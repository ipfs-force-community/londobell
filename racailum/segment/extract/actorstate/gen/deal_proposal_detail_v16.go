...
func init() {
	reg.MustRegisterPreCheck("DealProposalDetailedV16", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalDetailedV16", extractDealProposalDetailedV16)

}

func extractDealProposalDetailedV16(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market16.State) error {
	...
	state, err := lmarket.Load(adt16.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), head.Actor)
	if err != nil {
