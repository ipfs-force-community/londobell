...
func init() {
	reg.MustRegisterPreCheck("MultisigBalanceV8", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MultisigBalance
	}, nil)
	reg.MustRegisterRegularExtractor("MultisigBalanceV8", extractMultisigBalanceV8)

}

func extractMultisigBalanceV8(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *multisig8.State) error {
	epoch := head.Epoch
