...
func init() {
	reg.MustRegisterPreCheck("VerifRegV17", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("VerifRegV17", extractVerifRegV17)

}

func extractVerifRegV17(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *verifreg17.State) error {
	mapRoots := []struct {
		name string
		root cid.Cid
	}{
	...
	for mi := range mapRoots {
		...
		m, err := adt17.AsMap(ctx.D.ActorStore(ctx.C), root, builtin17.DefaultHamtBitwidth)
		if err != nil {
