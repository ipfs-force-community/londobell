...
func init() {
	reg.MustRegisterPreCheck("DealProposalDetailedV15", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalDetailedV15", extractDealProposalDetailedV15)

}

func extractDealProposalDetailedV15(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market15.State) error {
	...
	state, err := lmarket.Load(adt15.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), head.Actor)
	if err != nil {
