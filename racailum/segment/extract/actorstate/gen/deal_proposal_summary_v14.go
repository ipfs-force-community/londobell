...
func init() {
	reg.MustRegisterPreCheck("DealProposalSummaryV14", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalSummaryV14", extractDealProposalSummaryV14)

}

func extractDealProposalSummaryV14(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market14.State) error {
	deals, err := market14.AsDealProposalArray(adt14.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Proposals)
	if err != nil {
	...
	addProposal := func(p market14.DealProposal) {
		target := 0
		if p.VerifiedDeal {
			target = 1
		}
