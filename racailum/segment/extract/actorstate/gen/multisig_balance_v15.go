...
func init() {
	reg.MustRegisterPreCheck("MultisigBalanceV15", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MultisigBalance
	}, nil)
	reg.MustRegisterRegularExtractor("MultisigBalanceV15", extractMultisigBalanceV15)

}

func extractMultisigBalanceV15(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *multisig15.State) error {
	epoch := head.Epoch
