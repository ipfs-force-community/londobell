...
func init() {
	reg.MustRegisterPreCheck("DealProposalDetailedV7", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalDetailedV7", extractDealProposalDetailedV7)

}

// extract in chronological order, Otherwise deals created and deleted earlier will be left over
func extractDealProposalDetailedV7(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market7.State) error {
	...
	state, err := lmarket.Load(adt7.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), head.Actor)
	if err != nil {
		return fmt.Errorf("failed to load market actor state: %w", err)
	}

	deals, err := market7.AsDealProposalArray(adt7.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Proposals)
	if err != nil {
		return fmt.Errorf("load deal proposal array: %w", err)
	}

	dealsStateArr, err := market7.AsDealStateArray(adt7.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.States)
	if err != nil {
