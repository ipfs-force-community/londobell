...
func init() {
	reg.MustRegisterPreCheck("MarketFundsV11", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MarketFunds
	}, func(ctx *extract.Ctx) int {
		return ctx.Opts.StateRegular.MarketFundsTicks
	})
	reg.MustRegisterRegularExtractor("MarketFundsV11", extractMarketFundsV11)

}

func extractMarketFundsV11(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market11.State) error {
	...
	proposals, err := market11.AsDealProposalArray(actStore, st.Proposals)
	if err != nil {
		return fmt.Errorf("constrcut DealProposalArray: %w", err)
	}

	deal := &market11.DealProposal{}
