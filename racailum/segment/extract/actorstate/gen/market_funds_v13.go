...
func init() {
	reg.MustRegisterPreCheck("MarketFundsV13", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MarketFunds
	}, func(ctx *extract.Ctx) int {
		return ctx.Opts.StateRegular.MarketFundsTicks
	})
	reg.MustRegisterRegularExtractor("MarketFundsV13", extractMarketFundsV13)

}

func extractMarketFundsV13(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market13.State) error {
	...
	proposals, err := market13.AsDealProposalArray(actStore, st.Proposals)
	if err != nil {
		return fmt.Errorf("constrcut DealProposalArray: %w", err)
	}

	deal := &market13.DealProposal{}
