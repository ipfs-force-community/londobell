...
func init() {
	reg.MustRegisterPreCheck("MultisigBalanceV10", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MultisigBalance
	}, nil)
	reg.MustRegisterRegularExtractor("MultisigBalanceV10", extractMultisigBalanceV10)

}

func extractMultisigBalanceV10(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *multisig10.State) error {
	epoch := head.Epoch
