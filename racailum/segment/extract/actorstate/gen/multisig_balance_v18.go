...
func init() {
	reg.MustRegisterPreCheck("MultisigBalanceV18", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MultisigBalance
	}, nil)
	reg.MustRegisterRegularExtractor("MultisigBalanceV18", extractMultisigBalanceV18)

}

func extractMultisigBalanceV18(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *multisig18.State) error {
	epoch := head.Epoch
