...
func init() {
	reg.MustRegisterPreCheck("ClaimedPowerV15", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.ClaimedPower
	}, nil)
	reg.MustRegisterRegularExtractor("ClaimedPowerV15", extractClaimedPowerV15)

	schema.Register(
		schema.Model{
			Name: "claimed-power-v15",
			D: &model.ClaimedPower{
				Detail: &power15.Claim{},
			},
		},
	)
}

func extractClaimedPowerV15(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, pst *power15.State) error { // nolint: deadcode

	claims, err := adt15.AsMap(ctx.D.ActorStore(ctx.C), pst.Claims, builtin15.DefaultHamtBitwidth)

	if err != nil {
		return fmt.Errorf("construct adt.Map for Claims in *power15.State: %w", err)
	}
