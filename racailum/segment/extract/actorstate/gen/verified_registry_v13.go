...
func init() {
	reg.MustRegisterPreCheck("VerifRegV13", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("VerifRegV13", extractVerifRegV13)

}

func extractVerifRegV13(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *verifreg13.State) error {
	mapRoots := []struct {
		name string
		root cid.Cid
	}{
	...
	for mi := range mapRoots {
		...
		m, err := adt13.AsMap(ctx.D.ActorStore(ctx.C), root, builtin13.DefaultHamtBitwidth)
		if err != nil {
