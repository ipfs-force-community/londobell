...
func init() {
	reg.MustRegisterPreCheck("MarketFundsV15", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MarketFunds
	}, func(ctx *extract.Ctx) int {
		return ctx.Opts.StateRegular.MarketFundsTicks
	})
	reg.MustRegisterRegularExtractor("MarketFundsV15", extractMarketFundsV15)

}

func extractMarketFundsV15(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market15.State) error {
	...
	proposals, err := market15.AsDealProposalArray(actStore, st.Proposals)
	if err != nil {
		return fmt.Errorf("constrcut DealProposalArray: %w", err)
	}

	deal := &market15.DealProposal{}
