...
func init() {
	reg.MustRegisterPreCheck("DealProposalDetailedV5", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalDetailedV5", extractDealProposalDetailedV5)
...
func extractDealProposalDetailedV5(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market5.State) error {
	...
	state, err := lmarket.Load(adt5.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), head.Actor)
	if err != nil {
