...
func init() {
	reg.MustRegisterPreCheck("MultisigBalanceV7", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MultisigBalance
	}, nil)
	reg.MustRegisterRegularExtractor("MultisigBalanceV7", extractMultisigBalanceV7)

}

func extractMultisigBalanceV7(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *multisig7.State) error {
	epoch := head.Epoch
