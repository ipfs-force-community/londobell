...
func init() {
	reg.MustRegisterPreCheck("VerifRegV16", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("VerifRegV16", extractVerifRegV16)

}

func extractVerifRegV16(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *verifreg16.State) error {
	mapRoots := []struct {
		name string
		root cid.Cid
	}{
	...
	for mi := range mapRoots {
		...
		m, err := adt16.AsMap(ctx.D.ActorStore(ctx.C), root, builtin16.DefaultHamtBitwidth)
		if err != nil {
