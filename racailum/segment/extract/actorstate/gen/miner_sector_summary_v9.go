...
func init() {
	summaryDaysV9 = []abi.ChainEpoch{1, 7, 14, 30}
	for d := (miner9.MinSectorExpiration / builtin9.EpochsInDay); d <= (miner9.MaxSectorExpirationExtension / builtin9.EpochsInDay); d += 180 {
		summaryDaysV9 = append(summaryDaysV9, abi.ChainEpoch(d))
	}
	if (miner9.MaxSectorExpirationExtension / builtin9.EpochsInDay) > summaryDaysV9[len(summaryDaysV9)-1] {
		summaryDaysV9 = append(summaryDaysV9, abi.ChainEpoch(miner9.MaxSectorExpirationExtension/builtin9.EpochsInDay))
	}

	reg.MustRegisterPreCheck("MinerSectorSummaryV9", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("MinerSectorSummaryV9", extractMinerSectorSummaryV9)
}

var summaryDaysV9 []abi.ChainEpoch

func extractMinerSectorSummaryV9(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *miner9.State) error {
	if st.Sectors.Equals(emptyMinerStateV9.Sectors) {
		return nil
	}

	daysMax := summaryDaysV9[len(summaryDaysV9)-1]
	summaries := make([]*model.MinerSectorSummaryRange, 0, len(summaryDaysV9)+1)
	summariesInDays := make([]*model.MinerSectorSummaryRange, 0, int(daysMax+1))

	var prevDays abi.ChainEpoch
	for _, days := range summaryDaysV9 {
		current := &model.MinerSectorSummaryRange{
			LowerBound:              prevDays * builtin9.EpochsInDay,
			UpperBound:              days * builtin9.EpochsInDay,
			SectorCount:             0,
			DealCount:               0,
			V1SectorCount:           0,
	...
	sectors, err := miner9.LoadSectors(adt9.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Sectors)
	if err != nil {
	...
	err = sectors.ForEach(&out, func(n int64) error {
		...
		remainDays := remainDuration / builtin9.EpochsInDay
		...
		if len(out.DealIDs) == 0 {
			...
			mds = append(mds, model.MinerDealSector{
				...
				QAPower:            miner9.QAPowerForWeight(sectorSize, out.Expiration-out.Activation, out.DealWeight, out.VerifiedDealWeight),
				Miner:              head.Addr,
			})
		}
		return nil
