...
func init() {
	summaryDaysV14 = []abi.ChainEpoch{1, 7, 14, 30}
	for d := (miner14.MinSectorExpiration / builtin14.EpochsInDay); d <= (miner14.MaxSectorExpirationExtension / builtin14.EpochsInDay); d += 180 {
		summaryDaysV14 = append(summaryDaysV14, abi.ChainEpoch(d))
	}
	if (miner14.MaxSectorExpirationExtension / builtin14.EpochsInDay) > summaryDaysV14[len(summaryDaysV14)-1] {
		summaryDaysV14 = append(summaryDaysV14, abi.ChainEpoch(miner14.MaxSectorExpirationExtension/builtin14.EpochsInDay))
	}

	reg.MustRegisterPreCheck("MinerSectorSummaryV14", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("MinerSectorSummaryV14", extractMinerSectorSummaryV14)
}

var summaryDaysV14 []abi.ChainEpoch

func extractMinerSectorSummaryV14(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *miner14.State) error {
	if st.Sectors.Equals(emptyMinerStateV14.Sectors) {
		return nil
	}

	daysMax := summaryDaysV14[len(summaryDaysV14)-1]
	summaries := make([]*model.MinerSectorSummaryRange, 0, len(summaryDaysV14)+1)
	summariesInDays := make([]*model.MinerSectorSummaryRange, 0, int(daysMax+1))

	var prevDays abi.ChainEpoch
	for _, days := range summaryDaysV14 {
		current := &model.MinerSectorSummaryRange{
			LowerBound:              prevDays * builtin14.EpochsInDay,
			UpperBound:              days * builtin14.EpochsInDay,
			SectorCount:             0,
			DealCount:               0,
			V1SectorCount:           0,
	...
	sectors, err := miner14.LoadSectors(adt14.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Sectors)
	if err != nil {
		return fmt.Errorf("load sectors from adt store: %w", err)
	}

	var out miner14.SectorOnChainInfo
	...
	err = sectors.ForEach(&out, func(n int64) error {
		...
		remainDays := remainDuration / builtin14.EpochsInDay
		...
		if len(out.DealIDs) == 0 {
			...
			mds = append(mds, model.MinerDealSector{
				...
				QAPower:            miner14.QAPowerForWeight(sectorSize, out.Expiration-out.Activation, out.DealWeight, out.VerifiedDealWeight),
				Miner:              head.Addr,
			})
		}
		return nil
