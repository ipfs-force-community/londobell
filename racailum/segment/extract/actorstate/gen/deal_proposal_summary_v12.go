...
func init() {
	reg.MustRegisterPreCheck("DealProposalSummaryV12", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalSummaryV12", extractDealProposalSummaryV12)

}

func extractDealProposalSummaryV12(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market12.State) error {
	deals, err := market12.AsDealProposalArray(adt12.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Proposals)
	if err != nil {
	...
	addProposal := func(p market12.DealProposal) {
		target := 0
		if p.VerifiedDeal {
			target = 1
		}
