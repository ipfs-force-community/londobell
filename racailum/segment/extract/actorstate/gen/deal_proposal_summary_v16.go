...
func init() {
	reg.MustRegisterPreCheck("DealProposalSummaryV16", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalSummaryV16", extractDealProposalSummaryV16)

}

func extractDealProposalSummaryV16(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market16.State) error {
	deals, err := market16.AsDealProposalArray(adt16.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Proposals)
	if err != nil {
	...
	addProposal := func(p market16.DealProposal) {
		target := 0
		if p.VerifiedDeal {
			target = 1
		}
