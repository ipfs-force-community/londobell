...
func init() {
	reg.MustRegisterPreCheck("VerifRegV10", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("VerifRegV10", extractVerifRegV10)

}

func extractVerifRegV10(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *verifreg10.State) error {
	mapRoots := []struct {
		name string
		root cid.Cid
	}{
	...
	for mi := range mapRoots {
		...
		m, err := adt10.AsMap(ctx.D.ActorStore(ctx.C), root, builtin10.DefaultHamtBitwidth)
		if err != nil {
