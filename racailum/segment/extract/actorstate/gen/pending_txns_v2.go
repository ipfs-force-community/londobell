...
func init() {
	reg.MustRegisterPreCheck("PendingTxnsV2", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.PendingTxns
	}, func(ctx *extract.Ctx) int {
		return ctx.Opts.StateRegular.PendingTxnsTicks
	})
	reg.MustRegisterRegularExtractor("PendingTxnsV2", extractPendingTxnsV2)
...
func extractPendingTxnsV2(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *multisig2.State) error {
	...
	if err != nil {
		return fmt.Errorf("construct adt.Map for PendingTxns in *multisig2.State: %w", err)
	}

	out := &multisig2.Transaction{}
