...
func init() {
	reg.MustRegisterPreCheck("MultisigBalanceV11", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MultisigBalance
	}, nil)
	reg.MustRegisterRegularExtractor("MultisigBalanceV11", extractMultisigBalanceV11)

}

func extractMultisigBalanceV11(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *multisig11.State) error {
	epoch := head.Epoch
