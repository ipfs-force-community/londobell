...
func init() {
	reg.MustRegisterPreCheck("DealProposalSummaryV11", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("DealProposalSummaryV11", extractDealProposalSummaryV11)

}

func extractDealProposalSummaryV11(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market11.State) error {
	deals, err := market11.AsDealProposalArray(adt11.WrapStore(ctx.C, ctx.D.ActorStore(ctx.C)), st.Proposals)
	if err != nil {
	...
	addProposal := func(p market11.DealProposal) {
		target := 0
		if p.VerifiedDeal {
			target = 1
		}
