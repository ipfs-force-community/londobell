...
func init() {
	reg.MustRegisterPreCheck("MiningProfitabilityV17", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MiningProfitability
	}, nil)
	reg.MustRegisterRegularExtractor("MiningProfitabilityV17", extractMiningProfitabilityV17)

}

// VERCHECK

func extractMiningProfitabilityV17(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *reward17.State) error {
	...
	pwrState, ok := state.(*power17.State)
	if !ok {
		return fmt.Errorf("expecting *power17.State, got %T", pwrState)
	}

	qaPower := miner17.QAPowerForWeight(sectorSize32GiB, 180, big.Zero())

	storagePledge := miner17.ExpectedRewardForPower(st.ThisEpochRewardSmoothed, pwrState.ThisEpochQAPowerSmoothed, miner17.QAPowerMax(sectorSize32GiB), miner17.PreCommitDepositProjectionPeriod)
	...
	initPledge := miner17.InitialPledgeForPower(qaPower, st.ThisEpochBaselinePower, st.ThisEpochRewardSmoothed, pwrState.ThisEpochQAPowerSmoothed, head.CirculatingSupply.FilCirculating, epochsSinceRampStart, rampDurationEpochs)
	...
	detail := model.MiningProfitabilityDetail{
		ExpectedDayReward:         miner17.ExpectedRewardForPower(st.ThisEpochRewardSmoothed, pwrState.ThisEpochQAPowerSmoothed, qaPower, builtin17.EpochsInDay),
		...
		ProjectionOfFaultFee:      miner17.ExpectedRewardForPower(st.ThisEpochRewardSmoothed, pwrState.ThisEpochQAPowerSmoothed, qaPower, abi.ChainEpoch(builtin17.EpochsInDay*351)/100),
