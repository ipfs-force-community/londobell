...
func init() {
	reg.MustRegisterPreCheck("VerifRegV18", func(ctx *extract.Ctx) bool {
	...
	reg.MustRegisterRegularExtractor("VerifRegV18", extractVerifRegV18)

}

func extractVerifRegV18(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *verifreg18.State) error {
	mapRoots := []struct {
		name string
		root cid.Cid
	}{
	...
	for mi := range mapRoots {
		...
		m, err := adt18.AsMap(ctx.D.ActorStore(ctx.C), root, builtin18.DefaultHamtBitwidth)
		if err != nil {
