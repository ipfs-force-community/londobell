...
func init() {
	reg.MustRegisterPreCheck("MarketFundsV12", func(ctx *extract.Ctx) bool {
		return ctx.Opts.ZeroHourExtract.MarketFunds
	}, func(ctx *extract.Ctx) int {
		return ctx.Opts.StateRegular.MarketFundsTicks
	})
	reg.MustRegisterRegularExtractor("MarketFundsV12", extractMarketFundsV12)

}

func extractMarketFundsV12(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market12.State) error {
	...
	proposals, err := market12.AsDealProposalArray(actStore, st.Proposals)
	if err != nil {
		return fmt.Errorf("constrcut DealProposalArray: %w", err)
	}

	deal := &market12.DealProposal{}
