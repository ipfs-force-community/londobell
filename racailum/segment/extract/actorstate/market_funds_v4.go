...
func init() {
	mustRegisterRegularExtractor("MarketFundsV4", extractMarketFundsV4)

}

func extractMarketFundsV4(ctx *extract.Ctx, res *extract.Res, head *common.ActorHead, st *market4.State) error {
	...
	proposals, err := market4.AsDealProposalArray(actStore, st.Proposals)
	if err != nil {
		return fmt.Errorf("constrcut DealProposalArray: %w", err)
	}

	deal := &market4.DealProposal{}
